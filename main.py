...
def sample_from_trained_model(trained_model, device):
    trained_model.load_state_dict(torch.load('model.pth', map_location=device))
...
def run(train_dataset, dev_dataset, max_iter=1, device='cpu', plot=True, sample=True):
    ...
    if plot:
        trainer.plot()
    if sample:
        sample_from_trained_model(model, device)
...
def cpu_gpu_comparison(train_dataset, dev_dataset, max_iter=100):
    ...
    accel_device = get_accelerator_device()
    accel_time = None
    if accel_device != 'cpu':
        start = time.perf_counter()
        run(train_dataset, dev_dataset, max_iter=max_iter, device=accel_device, plot=False, sample=False)
        accel_time = time.perf_counter() - start
    else:
        print('No GPU backend available (neither CUDA nor MPS).')

    print(f'CPU {max_iter} iters elapsed: {cpu_time:.2f}s')
    if accel_time is not None:
        print(f'{accel_device.upper()} {max_iter} iters elapsed: {accel_time:.2f}s')

    return cpu_time, accel_time, accel_device


def gpu_full_run(train_dataset, dev_dataset, max_iter=20000):
    accel_device = get_accelerator_device()
    run(train_dataset, dev_dataset, max_iter=max_iter, device=accel_device, plot=True, sample=True)
    return accel_device
...
if __name__ == '__main__':
    ...
    # run the short benchmark first so runtime numbers can be reported in written answers.
    cpu_time, accel_time, accel_device = cpu_gpu_comparison(train_d, dev_d, max_iter=50)

    # TODO: run the full training on gpu
    # uncomment the following line to run
    # run the full gpu training pass to produce loss/ppl plots and text generations.
    full_run_device = gpu_full_run(train_d, dev_d, max_iter=20000)
    print(f'Runtime summary: cpu_time={cpu_time:.2f}s, accel_device={accel_device}, accel_time={accel_time}')
    print(f'Full training ran on: {full_run_device}')
